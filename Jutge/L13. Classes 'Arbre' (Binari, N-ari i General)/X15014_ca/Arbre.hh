...
public:
 ...
 public:
  // Imprimeix l'arbre en ASCII (girant-lo 90º cap a l'esquerra)
  void print_ascii(std::ostream& os = std::cout) const {
    print_aux(primer_node, 0, os);
  }

 private:
  // Helper recursiu: primer dret, després el node, després esquerre
  static void print_aux(node_arbre* p, int indent, std::ostream& os) {
    if (p == nullptr) return;
    // Imprimeixo el fill dret més amunt
    print_aux(p->segD, indent + 4, os);
    // Espais d'indentació
    for (int i = 0; i < indent; ++i) os << ' ';
    // Valor del node
    os << p->info << '\n';
    // Imprimeixo el fill esquerre més avall
    print_aux(p->segE, indent + 4, os);
  }
