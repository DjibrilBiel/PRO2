...
public:
  ...
  class iterator {
    ...
    {
      if ((pitem == &(plist->itemsup) and not mirror) or (pitem == plist->iteminf.next and mirror)) {
        mirror = not mirror;
      } else if (not mirror) {
        pitem = pitem->next;
      } else {
    ...
    /* Post: el p.i apunta a l'element anterior a E,
       el resultat és el p.i. */
    {
      if ((pitem == &(plist->itemsup) and mirror) or (pitem == plist->iteminf.next and not mirror)) {
        mirror = not mirror;
      } else if (mirror) {
        pitem = pitem->next;
      } else {
